        ...
        {filteredOrganizations.map((organization) => (
          <tr key={organization.id} className="border-b border-border last:border-b-0">
            <td className="px-4 py-3">
              <Link href={`/organizations/${organization.id}`} className="font-medium text-slate-900 hover:text-blue-600">{organization.name}</Link>
              <div className="mt-1 text-xs text-slate-500">{organization.budgetBand}</div>
            </td>
            <td className="px-4 py-3 text-sm text-slate-600">{labelForOwnerType(organization.kind)}</td>
            <td className="px-4 py-3"><StatusBadge value={organization.verificationStatus} /></td>
            <td className="px-4 py-3"><StatusBadge value={organization.documentationStatus} /></td>
            <td className="px-4 py-3 text-sm text-slate-600">{organization.projectsCount}</td>
            <td className="px-4 py-3 text-sm text-slate-600">{organization.membersCount}</td>
            <td className="px-4 py-3 text-sm text-slate-600">{formatDateTime(organization.lastActiveAt)}</td>
          </tr>
        ))}
      </DataTable>
    </SectionScaffold>
