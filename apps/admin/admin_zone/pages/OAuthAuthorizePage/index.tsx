...
import { approveOAuthAuthorization, getOAuthAuthorizePageData } from "@/admin_zone/api/oauth";
...
/**
 * WHY:   Admin OAuth consent must happen inside the admin app so approvals bind to the right session.
 * WHAT:  Renders the OAuth consent prompt for admins and handles approve/deny actions.
 * HOW:   Loads the flow prompt via the admin OAuth API and redirects on approval or denial.
 */
export default async function OAuthAuthorizePage({ flow }: OAuthAuthorizePageProps) {
  if (!flow) {
    redirect("/signin");
  }

  const { session, preview } = await getOAuthAuthorizePageData(flow);

  if (!preview.requiresConsent && preview.existingAuthorization) {
    const result = await approveOAuthAuthorization(flow);
  ...
  async function approveAuthorization(formData: FormData) {
    ...
    const result = await approveOAuthAuthorization(flowId);
    redirect(result.redirectUrl);
  }
  ...
  return (
    <AdminShell user={session.user}>
      <main className="min-h-[calc(100svh-12rem)] bg-[radial-gradient(circle_at_top,_rgba(37,99,235,0.06),_transparent_45%),linear-gradient(180deg,#f8fafc_0%,#ffffff_40%,#eef2ff_100%)] text-slate-900">
