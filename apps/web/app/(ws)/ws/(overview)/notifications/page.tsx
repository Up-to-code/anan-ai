    ...
    <div className="flex min-h-full flex-col">
      ...
      <div className="space-y-4 px-6 py-6 lg:px-8 lg:py-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="inline-flex border border-slate-200 bg-white p-1 dark:border-slate-800 dark:bg-slate-950">
            <Link
        ...
        <div className="grid gap-3">
          {filteredNotifications.length === 0 ? (
            <div className="border border-slate-200 bg-white p-6 text-sm font-bold text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">
            ...
            filteredNotifications.map((item) => (
              <Link
                key={item.id}
                href={item.href}
                className="block border border-slate-200 bg-white p-4 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 dark:border-slate-800 dark:bg-slate-950 dark:hover:bg-slate-900"
              >
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      {!item.isRead ? (
                        ...
                        <span aria-hidden="true" className="h-2 w-2 shrink-0 bg-slate-200 dark:bg-slate-700" />
                      )}
                      <div className="truncate text-sm font-black text-slate-950 dark:text-slate-100">
                        {item.title}
                      </div>
                    </div>
                    <div className="mt-2 line-clamp-2 text-sm font-medium leading-6 text-slate-600 dark:text-slate-300">
                      {item.summary}
                    </div>
                    <div className="mt-3 flex flex-wrap items-center gap-3 text-[10px] font-black uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">
