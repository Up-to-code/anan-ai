...
export default function OfferCards({ items }: { items: OfferMarketplaceItem[] }) {
  return (
        ...
        >
          <div className="flex min-w-0 flex-1 flex-col gap-3 relative">
            <div className="flex items-center gap-3">
              ...
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-500 border border-slate-200 bg-slate-50 px-3 py-1">{item.demandLabel ?? item.ownerLabel}</div>
            </div>
            <p className="text-sm font-medium leading-relaxed text-slate-500 max-w-3xl"><span className="font-bold text-slate-900 border-b border-slate-200 pb-0.5">{item.project.title}</span> • {item.summary}</p>
          ...
          <div className="flex shrink-0 flex-col items-start gap-4 sm:items-end w-full md:w-auto mt-4 md:mt-0">
            {item.broker ? <div className="w-full md:w-auto"><BrokerPresenceChip broker={item.broker} /></div> : null}
