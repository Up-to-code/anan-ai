  ...
  if (rows.length === 0) {
    return (
      <MarketPanel title={title} description={description}>
        <div className="py-10 text-center text-sm text-slate-500">لا توجد مناطق مطابقة لهذا النطاق.</div>
      </MarketPanel>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-200 text-right">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((row) => (
              <tr key={`${row.city}-${row.area}`} className="align-top text-sm text-slate-700">
                {showCityColumn ? <td className="py-3">{row.city}</td> : null}
                <td className="py-3 font-medium text-slate-950">{row.area}</td>
                <td className="py-3">
