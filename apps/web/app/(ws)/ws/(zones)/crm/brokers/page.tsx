...
function BrokerMemberIdentity({ member }: Pick<BrokerMemberCardProps, "member">) {
  return (
    <div className="flex items-start gap-4">
      <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden bg-slate-100 text-xl font-black text-slate-400">{member.name.slice(0, 1)}</div>
...
function BrokerMemberCard({ member, authUserId }: BrokerMemberCardProps) {
  ...
  return (
    <div className="group relative border border-slate-200 bg-white p-6 transition hover:border-blue-200">
      {canMessage ? (
        <Link
