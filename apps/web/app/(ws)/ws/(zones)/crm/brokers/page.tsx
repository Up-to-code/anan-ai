...
function BrokerMemberIdentity({ member }: Pick<BrokerMemberCardProps, "member">) {
  return (
    <div className="flex items-start gap-4">
      <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-slate-100 text-xl font-black text-slate-400">{member.name.slice(0, 1)}</div>
    ...
    <div className="mt-6 space-y-4">
      ...
      <div className="flex items-center gap-3 border-t border-slate-100 pt-4">
        <div className="flex-1">
          ...
          <div className="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-black text-blue-700">
...
function BrokerMemberCard({ member, authUserId }: BrokerMemberCardProps) {
  ...
  return (
    <div className="group relative rounded-lg border border-slate-200 bg-white p-6 transition hover:border-blue-200">
      {canMessage ? (
        <Link
          href={`/ws/inbox?startUserId=${encodeURIComponent(member.authUserId)}`}
          className="absolute left-4 top-4 inline-flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-700 transition hover:border-blue-600 hover:text-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-200"
      ...
      actions={
        <div className="flex items-center gap-2">
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-3 text-xs font-black tracking-[0.18em] text-slate-700 transition hover:border-emerald-600 hover:bg-emerald-50 hover:text-emerald-700"
          >
...
function OrganizationSummaryCard({ name, membersCount, invitesCount }: { name: string; membersCount: number; invitesCount: number }) {
  return (
    <div className="mb-6 rounded-lg border border-slate-200 bg-[linear-gradient(180deg,#ffffff_0%,#f8fbff_100%)] p-5">
      <div className="text-xs font-black tracking-[0.22em] text-blue-700">المنظمة الحالية</div>
      <div className="mt-2 text-2xl font-black text-slate-950">{name}</div>
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      <div className="px-6 py-6 lg:px-8 lg:py-8">
        ...
        {members.length === 0 ? (
          <div className="rounded-lg border border-dashed border-slate-200 py-12 text-center text-sm font-bold text-slate-500">
