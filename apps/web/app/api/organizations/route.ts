...
import { bootstrapCurrentOrganizationFromClerk } from "@/server/domains/auth/organizations/service";

/**
 * WHY:   Custom Clerk organization creation still needs one app-owned bootstrap step after the org becomes active.
...
export async function POST(request: Request) {
  try {
    ...
    const organization = await bootstrapCurrentOrganizationFromClerk(body);
