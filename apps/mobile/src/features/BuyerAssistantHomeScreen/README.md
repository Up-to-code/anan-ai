...
- `ActivePropertyComposerCard.tsx` renders the compact property helper shown above the composer when chat is anchored to one property.
...
- `propertyPrompt.ts` owns the draft-shaping helpers that merge active-property context into the user's prompt once.
