...
/**
 * WHY:   The upgraded client thread needs one compatibility layer while live assistant payloads still use the older property/card structure.
 * WHAT:  Converts the existing deterministic assistant artifacts into the new client AG UI turn model.
 * HOW:   Creates a shortlist card first, then appends mapped UI cards in response order.
 */
