...
const ENV_KEYS = ["ANAN_WEB_URL", "ANAN_ADMIN_URL", "ANAN_MOBILE_URL", "SITE_URL"] as const;
...
describe("OAuth consent routing", () => {
  ...
  it("parses only known source app values", () => {
    ...
    expect(parseOAuthSourceApp("mobile")).toBe("mobile");
  ...
  it("resolves the consent base URL using app-specific envs", () => {
    ...
    process.env.ANAN_MOBILE_URL = "https://m.anan.local";
    ...
    expect(resolveOAuthConsentBaseUrl(request, "mobile")).toBe("https://m.anan.local");
  ...
  it("falls back to the web base URL when the app origin is missing", () => {
    ...
    expect(resolveOAuthConsentBaseUrl(request, "mobile")).toBe("https://web.anan.local");
  });
