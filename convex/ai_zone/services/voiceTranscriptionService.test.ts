...
const mockedRefs = vi.hoisted(() => ({
  apiRefs: {
    "ai_zone/assistantWorkspace": { getThread: Symbol("assistantWorkspace.getThread") },
    "shared_logic/lib/storage": { getUrl: Symbol("storage.getUrl") },
  },
}));
...
describe("voiceTranscriptionService", () => {
  ...
  it("transcribes a stored voice note successfully", async () => {
    ...
    const runQuery = vi.fn(async (ref: unknown) => {
      if (ref === apiRefs["ai_zone/assistantWorkspace"].getThread) {
        return { thread: { _id: "thread_1" } };
      }
      if (ref === apiRefs["shared_logic/lib/storage"].getUrl) {
        return "https://storage.example.com/audio.webm";
      }
      return null;
    });
