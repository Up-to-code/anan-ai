...
export const generateResponseWorkflow = workflow.define({
  ...
  handler: async (step, { threadId, promptMessageId, channel }) => {
    ...
    const sendMessageAction = apiRefs["ai_zone/assistant"].sendMessage;
    ...
    const messageDoc = await step.runQuery(
      internalRefs["ai_zone/assistant"]._getMessageContent,
...
export const startGenerateResponseWorkflow = internalAction({
  ...
  handler: async (ctx, actionArgs) => {
    const wf = internalRefs["ai_zone/agents/shared/workflows"].generateResponseWorkflow;
