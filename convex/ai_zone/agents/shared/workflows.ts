...
export const startGenerateResponseWorkflow = internalAction({
  ...
  handler: async (ctx, actionArgs) => {
    const ia = internal as Record<string, unknown>;
    const agents = ia.agents as Record<string, unknown> | undefined;
    const workflows = agents?.workflows as Record<string, unknown> | undefined;
    const wf = workflows?.generateResponseWorkflow;
