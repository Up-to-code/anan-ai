/**
 * Shared utilities: tokenize, sanitize, truncate, transformer.
...
/** Tokenize query for search – tokens + location phrases. */
...
const QUERY_STOPWORDS = new Set([
  "the", "a", "an", "and", "or", "in", "on", "at", "for", "to", "of", "with", "is", "are", "be",
  "في", "فيها", "من", "إلى", "عن", "على", "مع", "هذا", "هذه", "ذلك",
]);
...
function extractLocationPhrases(query: string): string[] {
  ...
  if (cityMatch) phrases.push(...cityMatch.map((p) => p.toLowerCase()));
  const areaMatch = normalized.match(
    /(?:al|northern|southern|eastern|western|حي|شارع)\s+(\p{L}+)/gu,
  );
  if (areaMatch) phrases.push(...areaMatch.map((p) => p.toLowerCase()));
