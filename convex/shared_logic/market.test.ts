...
describe("shared market snapshot", () => {
  it("combines inventory, research, and search demand for city and area ranking", async () => {
    ...
    await t.run(async (ctx) => {
      ...
      await ctx.db.insert("searchLogs", {
        query: "شقق الملقا الرياض",
        stage: "completed",
        status: "completed",
      });
      await ctx.db.insert("searchLogs", {
        query: "فلل حطين الرياض",
        stage: "completed",
        status: "completed",
      });
    ...
    const snapshot = (await t.query(apiRefs["shared_logic/market"].getMarketSnapshot as never, {
      city: "الرياض",
      area: "الملقا",
      windowDays: 90,
    } as never)) as any;
