...
import { requireOrganizationMembership } from "./repositories/membership";
...
const organizationAssetMimeValidator = v.union(
...
function assertVerifiedAssetInput(args: { mime: string; sha256: string; size: number }) {
  if (args.size <= 0 || !Number.isFinite(args.size)) {
    throw new ConvexError({ code: "INVALID_UPLOAD", message: "Asset size must be positive" });
  }
  if (!/^[a-f0-9]{64}$/u.test(args.sha256)) {
    throw new ConvexError({ code: "INVALID_UPLOAD", message: "Asset checksum is invalid" });
  }
  if (!["image/jpeg", "image/png", "image/webp", "application/pdf"].includes(args.mime)) {
    throw new ConvexError({ code: "INVALID_UPLOAD", message: "Asset MIME type is not allowed" });
  }
}
...
export const upsertOrganizationAsset = mutation({
  args: {
    ...
    name: v.string(),
    size: v.number(),
    mime: organizationAssetMimeValidator,
    sha256: v.string(),
