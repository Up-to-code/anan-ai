...
import {
  ...
  requireOrganizationScopes,
} from "../../_core/security/authOrganizations";
import { findProfileByAuthUserId } from "./repositories/core";
...
export const updateCurrentOrganizationProfile = mutation({
  ...
  handler: async (ctx, args) => {
    const actor = await requireSession(ctx);
    const organization = await requireOrganizationScopes(ctx, ["org:workspace:manage"]);
