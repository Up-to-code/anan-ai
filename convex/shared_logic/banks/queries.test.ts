...
it("getBySlug returns null when bank does not exist", async () => {
  ...
  const result = await t.query(api.shared_logic.banks.queries.getBySlug as never, {
...
it("list returns empty array when no banks", async () => {
  const t = convexTest(schema, modules);
  const result = await t.query(api.shared_logic.banks.queries.list as never, {} as never);
  expect(result).toEqual([]);
});

it("getBundles returns empty array when no banks", async () => {
  const t = convexTest(schema, modules);
  const result = await t.query(api.shared_logic.banks.queries.getBundles as never, {} as never);
  expect(result).toEqual([]);
});
