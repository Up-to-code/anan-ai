...
it("ensureWhatsAppUser creates a new user", async () => {
  ...
  const id = await t.mutation(api.shared_logic.users.whatsapp.ensureWhatsAppUser as never, {
    userId: "966501234567",
    displayName: "Ahmed",
  } as never);
...
it("ensureWhatsAppUser returns existing user ID on duplicate", async () => {
  const t = convexTest(schema, modules);
  const id1 = await t.mutation(api.shared_logic.users.whatsapp.ensureWhatsAppUser as never, {
    userId: "966501234567",
    displayName: "Ahmed",
  } as never);
  const id2 = await t.mutation(api.shared_logic.users.whatsapp.ensureWhatsAppUser as never, {
    userId: "966501234567",
    displayName: "Ahmed",
  } as never);
  expect(id1).toBe(id2);
