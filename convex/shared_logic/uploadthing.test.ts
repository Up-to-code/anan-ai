...
describe("uploadthing tracking", () => {
  it("upserts files with tenant folder and metadata", async () => {
    ...
    await t.mutation(
      apiRefs["shared_logic/uploadthing"].trackUploadthingFile as never,
      {
  ...
  it("lists files scoped to the tenant and category", async () => {
    ...
    await t.query(
      apiRefs["shared_logic/uploadthing"].listCurrentTenantFiles as never,
      {
