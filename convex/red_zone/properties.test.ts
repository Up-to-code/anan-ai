...
it("creates RED properties with derived search text and lists by RED id", async () => {
  ...
  const property = await t.query(api.red_zone.properties.getById, {
    id: propertyId,
  } as never);
  expect((property as any)?.searchText).toContain("Sea view");
  expect((property as any)?.publicationState).toBe("draft");

  const page = await t.query(api.red_zone.properties.listByRedId, {
    REDId: redId,
...
it("counts overview stats by RED id", async () => {
  ...
  const overview = await t.query(api.red_zone.overview.countPropertiesByRedId, {
    REDId: redId,
  } as never);
