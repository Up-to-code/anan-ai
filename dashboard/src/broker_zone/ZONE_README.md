...
This folder is strictly an isolated **Fortress** assigned to real estate broker workflows. 
...
2. **Error Isolation:** If modifying top-level layouts, ensure you do not break the `BrokerZoneErrorBoundary` wrapping `App.tsx`.
3. **Strict API Location:** All new data-fetching hooks (e.g., getting deals, searching property) MUST reside in the `api/` directory.
